...
class AmarakoshaParser:
    ...
    def parse_lines(
        ...
        try:
            ...
            self.context.add_error(
                (
                    "System Operational Interruption: "
                    f"{str(exc)}"
                ),
                severity="fatal",
            )
    ...
    def _engine_loop(
        ...
        for raw_line in lines:
            ...
            result = ClassificationResult(
                line_type=line_type,
                content=(
                    self.context.current_line.strip()
                ),
            )
    ...
    def _handle_kanda(
        ...
        num = StructureNumbering.next_kanda_number(
            self.context.book,
        )
    ...
    def _handle_varga(
        ...
        num = StructureNumbering.next_varga_number(
            self.context.current_kanda,
        )
    ...
    def _handle_verse(
        ...
        num = StructureNumbering.next_verse_number(
            self.context.current_varga,
        )
    ...
    def _handle_unknown(
        ...
        self.context.add_error(
            (
                "Lexical analysis unknown token match: "
                f"{result.content}"
            ),
            severity="warning",
        )
    ...
    def _finalize_pipeline(
        ...
        warnings = (
            ParserValidator.validate_completion(
                self.context.book,
            )
        )
