...
• Explicitly finalize successful AcquisitionResult objects.
...
class DefaultSourceAcquirer:
    ...
    def acquire(
        ...
        Success is explicit.  A successful execution must call
        ``result.mark_success()`` before returning.

        This is important because AcquisitionResult starts with:

            success = False

        and does not infer success merely from the absence of errors.
        """
        ...
        if manifest is None:
            # AcquisitionResult requires a CorpusSource, so there is
            # no valid result object that can be constructed here.
            raise ValueError(
        ...
        try:
            ...
            if (
                ...
                result.add_error(
                    ...
                    f"expected size: {manifest.expected_size}; "
    ...
    def _prepare_destination(
        ...
        An existing destination directory is valid.

        ``overwrite_existing`` applies to the destination file,
    ...
    def _download_from_sources(
        ...
        A failed URL does not immediately fail the acquisition;
        the next URL is attempted.
        """
