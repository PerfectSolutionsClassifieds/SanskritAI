...
class MonierWilliamsSourceParser:
    ...
    The parser also supports an injected ``acquirer`` and/or custom
    delimited parser for compatibility with acquisition tests.
    """
    ...
    def parse(
        ...
        if source_text is None:
            ...
            if hasattr(self.acquirer, "read"):
                source_text = self.acquirer.read()
            elif hasattr(self.acquirer, "acquire"):
...
class _TaggedMonierWilliamsParser:
    ...
    def parse(
        ...
        for line in source_text.splitlines():
            ...
            if not inside:
                raise ValueError(
                    "Source content encountered outside an MW record"
                )
