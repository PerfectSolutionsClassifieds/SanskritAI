...
A. What exactly enters Amarakośa?
B. What exactly leaves the parser?
C. What does the current builder layer actually construct?
D. What does Synset contain?
E. What does Varga contain?
F. What lexical information is represented by Lexeme?
G. What canonical fields are available?
H. Which fields have a direct mapping?
I. Which fields require metadata/provenance?
J. Which fields currently have no canonical destination?
...
def main() -> None:
    ...
    show_signature(parser_cls, "AmarakoshaParser")
    show_source(parser_cls.parse, "AmarakoshaParser.parse")
    ...
    for name in ("SynsetRecord", "VargaRecord"):
        cls = components[name]
    ...
    for name in ("SynsetBuilder", "VargaBuilder"):
        cls = components[name]

        show_signature(cls, name)
        show_source(cls, name)

        print("\nabstract:", bool(inspect.isabstract(cls)))
        print(
            "abstract methods:",
    ...
    importer_cls = components["AmarakoshaImporter"]

    show_signature(importer_cls, "AmarakoshaImporter")
    show_source(importer_cls.import_source, "AmarakoshaImporter.import_source")
    ...
    registry_cls = components["AmarakoshaRegistry"]

    show_signature(registry_cls, "AmarakoshaRegistry")
    show_source(registry_cls.register_many, "AmarakoshaRegistry.register_many")
    show_source(registry_cls.vargas, "AmarakoshaRegistry.vargas")
    show_source(registry_cls.synsets, "AmarakoshaRegistry.synsets")
    ...
    lexeme_metadata_cls = components["LexemeMetadata"]

    show_signature(lexeme_cls, "Lexeme")
    show_signature(lexeme_metadata_cls, "LexemeMetadata")
    ...
    entry_cls = components["CanonicalDictionaryEntry"]

    show_signature(entry_cls, "CanonicalDictionaryEntry")
    show_dataclass_fields(
        entry_cls,
        "CanonicalDictionaryEntry",
    )
    ...
    sense_cls = components["CanonicalDictionarySense"]

    show_signature(sense_cls, "CanonicalDictionarySense")
    show_dataclass_fields(
        sense_cls,
        "CanonicalDictionarySense",
    )
    ...
    lexicon_cls = components["CanonicalLexicon"]

    show_signature(lexicon_cls, "CanonicalLexicon")
    show_public_members(lexicon_cls, "CanonicalLexicon")

    # -------------------------------------------------------------
    # 11. Explicit mapping evidence
    # -------------------------------------------------------------

    heading("11. PRELIMINARY FIELD MAPPING EVIDENCE")
    ...
    print(
        ...
        f"{'CANONICAL TARGET':45} | DECISION"
    )
    print("-" * 120)
    ...
    heading("12. BATCH 5A DECISION GATE")
    ...
    print(
        "  3. AmarakoshaImporter currently parses but does not "
        "perform builder/registry integration."
    )

    print(
        "  4. Therefore the current production Amarakośa pipeline "
        "does not yet produce a complete Synset/Varga runtime graph."
    )

    print(
        "  5. The canonical model is already available and should "
        "not be changed merely to accommodate this incomplete path."
    )
    ...
    print(
        "  Batch 5C — determine whether the existing Amarakośa "
        "builder/importer layer must first be completed."
    )
