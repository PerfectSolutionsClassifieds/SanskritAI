...
class TestTransformer(AbstractLexicalTransformer):
    """
    Minimal concrete transformer used to exercise the abstract transformer
    contract and its batch/diagnostic helpers.
    """
...
def test_identifier_defaults_to_concrete_class_name():
    ...
    assert transformer.identifier == "TestTransformer"
...
def test_string_representation_contains_class_and_resource():
    ...
    assert str(transformer) == (
        "TestTransformer(resource='Monier-Williams')"
    )
