...
def test_default_repository_has_no_rules_without_explicit_configuration():

    repository = DefaultSandhiRepository()

    assert repository.all() == repository.rule_set
    assert repository.count == len(repository.all())
    assert repository.count == 0
