...
def test_lexical_source_strips_string_fields():
    ...
    assert source.language == "sanskrit"
    assert source.script == "devanagari"
    assert source.description == "Sanskrit dictionary"
...
def test_has_version():
    source_without_version = LexicalSource(
        source_id="mw",
        name="Monier-Williams",
    )

    source_with_version = LexicalSource(
        source_id="mw",
        name="Monier-Williams",
        version="1899",
    )

    assert source_without_version.has_version is False
    assert source_with_version.has_version is True


def test_has_description():
    source_without_description = LexicalSource(
        source_id="mw",
        name="Monier-Williams",
    )

    source_with_description = LexicalSource(
        source_id="mw",
        name="Monier-Williams",
        description="Sanskrit-English dictionary",
    )

    assert source_without_description.has_description is False
    assert source_with_description.has_description is True


def test_has_url():
    source_without_url = LexicalSource(
        source_id="mw",
        name="Monier-Williams",
    )

    source_with_url = LexicalSource(
        source_id="mw",
        name="Monier-Williams",
        url="https://example.org/mw",
    )

    assert source_without_url.has_url is False
    assert source_with_url.has_url is True
