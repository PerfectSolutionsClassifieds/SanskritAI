...
def make_source_record() -> MonierWilliamsSourceRecord:
    """
    Construct one deterministic acquisition-stage
    Monier-Williams source record.

    This deliberately starts at the acquisition boundary.
    """
...
def make_canonical_lexicon(
    ...
    """
    Build the canonical lexicon boundary from one
...
def test_source_record_to_adapter_record_boundary():
    ...
    The adapter must normalize the acquisition representation
    without constructing canonical knowledge objects.
    """
...
def test_canonical_lexicon_to_knowledge_repository_to_domain_repository():
    ...
    The canonical knowledge repository must own the lexical
    state, while DefaultLexicalRepository acts only as the
    domain-facing adapter.
    """
    ...
    lexicon = make_canonical_lexicon(entry)
...
def test_complete_monier_williams_canonical_knowledge_flow():
    ...
    # ---------------------------------------------------------
    # 3. Normalized lexical representation → canonical
    # ---------------------------------------------------------
    ...
    assert result.canonical_source.source_id == "monier-williams
