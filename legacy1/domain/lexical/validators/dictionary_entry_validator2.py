...
This validator verifies the integrity of a DictionaryEntry and its
metadata. It does not attempt to determine whether the lexical or
linguistic content is correct.
"""
...
class DictionaryEntryValidator(
    ...
    def supports(
        ...
        """
        Return True when the object is a canonical DictionaryEntry.
        """
    ...
    def validate(
        ...
        """
        Validate one canonical DictionaryEntry.
        """
        ...
        if self.is_blank(metadata.dictionary_name):
            issues.append(
                self.warning(
                    ...
                    message="Dictionary entry dictionary_name is empty.",
                    suggestion=(
        ...
        if not isinstance(metadata.transliteration, str):
            issues.append(
                self.error(
                    ...
                    suggestion=(
                        "Provide transliteration as text."
                    ),
                )
            )
        ...
        if metadata is None:
            issues.append(
                self.error(
                    code="DIC010",
                    field="metadata",
                    message="Dictionary entry metadata must not be None.",
                    suggestion=(
                        "Provide a DictionaryEntryMetadata instance."
                    ),
                )
            )
