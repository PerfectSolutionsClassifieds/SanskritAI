...
class ReaderSession:
    ...
    def history_count(self) -> int:
        return self.history.history_count
    def open(
        self=None,
        position: ReaderPosition | None = None,
        *,
        ...
        Instance-style calls establish a new browsing root.
        """
        ...
        if not isinstance(engine, ReaderEngine):
            raise TypeError(
                "ReaderSession.open() requires a ReaderEngine."
            )
        if position is None:
            raise ValueError(
                "ReaderSession.open() requires a ReaderPosition."
            )
    ...
    def set_position(
        ...
        """
        Establish a new browsing root on the existing session.
    ...
    def _copy_history(self) -> ReaderSessionHistory:
        """
        Create a completely independent copy of the current history state.
        ReaderSessionHistory is a dataclass containing only ReaderPosition
        objects and private stacks, so deepcopy preserves the exact browser
        navigation state without mutating the original session.
        """
    ...
    def display_text(self) -> str:
        if self._result is not None and hasattr(
            self._result,
            "display_text",
        ):
