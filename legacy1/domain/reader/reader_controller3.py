...
class ReaderController:
    ...
    #     """Return the actual number of entries in the underlying reader history."""
    #     history = self.session.history
    #     count = getattr(history,"history_count",None)
    #     if count is None:
    #         count = getattr(history,"count",0)
    #     return int(count)

    # @property
    # def history_count(self) -> int:
    #     """Return the controller-visible history count, including the current position."""
    #     count = self.session.history_count
    #     if isinstance(count,int):
    #         return count + (1 if self.session.current_position is not None else 0)
    #     return 0
