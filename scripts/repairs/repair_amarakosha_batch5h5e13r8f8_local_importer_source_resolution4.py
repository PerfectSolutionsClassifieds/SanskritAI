...
This repair:
    ...
    - uses AST-aware validation so documentation strings are ignored

The repair is intentionally idempotent.

Important
---------
The previous validator incorrectly treated documentation such as:

    manifest.source.local_path

inside a method docstring as executable direct path resolution.

This version validates executable AST nodes instead of performing
raw substring checks against complete method source.
"""
...
def executable_body(
    ...
    """
    Return method statements excluding the leading docstring.

    This is important because architecture/contract documentation
    may legitimately mention manifest.source.local_path or
    manifest.metadata["source_path"] without performing those
...
def executable_nodes(
    ...
    """
    Yield executable AST nodes after excluding the method docstring.
    """
...
def insert_resolver_method(
    ...
    Insertion point:
        immediately before the first existing method.

    Existing helper methods remain untouched.
    """
...
def replace_methods(
    ...
    Replacements are performed from bottom to top so original
...
def direct_local_path_access_exists(
    ...
    """
    Detect executable direct access to .local_path.
    ...
    The validator is intentionally conservative: any executable
    Attribute node named local_path is treated as direct access.
    """
...
def direct_get_metadata_source_path_exists(
    ...
    """
    Detect executable calls to get_metadata("source_path").
    ...
    for node in executable_nodes(
        ...
        if (
            ...
            and argument.value == "source_path"
        ):
            return True
...
def verify_updated_contract(
    ...
    Validation is AST-based and therefore ignores documentation
    strings when checking executable behavior.
    """
    ...
    # --------------------------------------------------------------
    # Resolver must use canonical source
    # --------------------------------------------------------------

    resolver_has_manifest_source = any(
        isinstance(node, ast.Attribute)
    ...
    if not resolver_has_manifest_source:
        raise RuntimeError(
            "_resolve_source_path() does not inspect "
            "manifest.source."
        )

    resolver_has_local_path = any(
        isinstance(node, ast.Attribute)
        and node.attr == "local_path"
        for node in executable_nodes(
            resolver
    ...
    if not resolver_has_local_path:
        raise RuntimeError(
            "_resolve_source_path() does not inspect "
    ...
    # --------------------------------------------------------------
    # supports() must delegate to resolver
    # --------------------------------------------------------------
    ...
    # --------------------------------------------------------------
    # download() must delegate to resolver
    # --------------------------------------------------------------
...
def main() -> None:
    ...
    # --------------------------------------------------------------
    # Write production only after validation
    # --------------------------------------------------------------
