...
def method_source(
    ...
    """
    Return a method's source with class indentation removed.

    Important
    ---------
    ast.parse() cannot parse a standalone method that still has
    indentation inherited from its containing class.

    Therefore textwrap.dedent() is deliberately applied here.
    """
    ...
    # Critical fix:
    # remove indentation inherited from the class body.
...
def has_call(
    ...
    """
    Parse a standalone function/method safely.

    The input is expected to have already been dedented
    by method_source(), but dedent again defensively.
    """
...
def contains_attribute_access(
    ...
    This helper also defensively dedents the source.
    """
...
def physical_same_path(
    a: Path,
    b: Path,
) -> bool:
    try:
        return (
            a.resolve(strict=False)
            == b.resolve(strict=False)
        )
...
banner(
    "13R-8F-9 — Local Acquisition Lifecycle & "
    "Destination-Safety Audit"
)
...
for path in production_paths:
    ...
    if path.exists():
        ...
        print(
            f"WARNING: missing production file: {path}"
        )
...
banner(
    "2. LocalFileImporter structural contract"
)
...
resolver_has_metadata_fallback = (
    '"source_path"' in local_source
    or "'source_path'" in local_source
)

assert (
    resolver_has_local_path
    or resolver_has_metadata_fallback
), (
    "Resolver does not reference "
    "canonical local_path or compatibility source_path"
)
...
assert has_call(
    ...
    "supports() does not delegate "
    "to _resolve_source_path()"
)
...
print(
    "supports() delegation: PASS"
)

print(
    "download() delegation: PASS"
)
...
banner(
    "3. BaseDownloader lifecycle contract"
)

base_source = BASE_DOWNLOADER.read_text(
    encoding="utf-8"
)
...
for method_name, source in (
    ...
    for helper in base_helpers:
        ...
        print(
            ...
            f"{'PASS' if used else 'NOT DIRECTLY USED'}"
        )
...
banner(
    "5. AcquisitionResult lifecycle contract"
)

result_source = (
    ACQUISITION_RESULT.read_text(
        encoding="utf-8"
    )
)
...
for method_name in result_methods:
    ...
    if f"def {method_name}" in result_source:
        ...
        print(
            f"{method_name}(): PRESENT"
        )

    else:

        print(
            f"WARNING: {method_name}(): NOT FOUND"
        )
...
acquirer_source = (
    DEFAULT_SOURCE_ACQUIRER.read_text(
        encoding="utf-8"
    )
)
...
if "LocalFileImporter" in acquirer_source:
    ...
    print(
        "LocalFileImporter reference: PRESENT"
    )

else:

    print(
        "BLOCKER: LocalFileImporter reference missing"
    )
...
if "supports(" in acquirer_source:
    ...
    print(
        "Importer capability dispatch evidence: PRESENT"
    )
...
banner(
    "7. Amarakośa canonical artifact integrity"
)
...
print(
    f"Artifact : {AMARAKOSHA_ARTIFACT}"
)
...
banner(
    "8. Production Amarakośa manifest"
)
...
print(
    f"destination      : {manifest.destination_directory}"
)

print(
    f"expected_filename: {manifest.expected_filename}"
)

print(
    f"expected_size    : {manifest.expected_size}"
)
...
print(
    f"overwrite        : {manifest.overwrite_existing}"
)

print(
    f"urls             : {manifest.urls}"
)

print(
    f"mirrors          : {manifest.mirrors}"
)
...
if manifest.destination_directory is None:
    ...
    print(
        "BLOCKER: production manifest has "
    ...
    if same_path:
        ...
        print(
            "The production manifest resolves the "
            "local source and acquisition destination "
            "to the same physical file."
        )

        print(
            "This must be resolved at the "
            "manifest/lifecycle level before production "
...
# ==========================================================================
# 10. Isolated LocalFileImporter runtime
# ==========================================================================
...
with TemporaryDirectory() as temp_dir:
    ...
    # Never mutate the production manifest.
    ...
    if not supports:
        ...
        raise RuntimeError(
            "LocalFileImporter unexpectedly "
            "rejected canonical local source"
        )
    ...
    if not result.downloaded_files:
        ...
        raise RuntimeError(
            "Successful local acquisition "
            "produced no downloaded_files"
        )
    ...
    assert copied_size == artifact_size, (
        "Copied artifact size mismatch"
    )

    assert copied_hash == artifact_hash, (
        "Copied artifact SHA256 mismatch"
    )
...
banner(
    "11. Production artifact immutability"
)
...
assert post_size == artifact_size, (
    "Production artifact size changed "
    "during read-only audit"
)

assert post_hash == artifact_hash, (
    "Production artifact hash changed "
    "during read-only audit"
)
...
if manifest.destination_directory is not None:
    ...
    if physical_same_path(
        ...
        print(
            "AUDIT COMPLETE — lifecycle code is "
            "structurally healthy, but production "
            "Amarakośa acquisition destination is "
            "source-identical."
        )
        ...
        print(
            "Determine whether the production manifest "
            "is intended to be an acquisition-registration "
            "manifest only, or whether acquire() is expected "
            "to physically copy the artifact."
        )
    ...
    print(
        "13R-8F-9 COMPLETE — "
        "manifest destination requires review."
    )
