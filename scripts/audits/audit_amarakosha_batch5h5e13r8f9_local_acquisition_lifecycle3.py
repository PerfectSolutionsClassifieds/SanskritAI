...
def contains_attribute_access(source: str, attribute: str) -> bool:
    tree = ast.parse(source)
...
banner("13R-8F-9 — Local Acquisition Lifecycle & Destination-Safety Audit")
...
local_source = method_source(LOCAL_IMPORTER, "LocalFileImporter", "_resolve_source_path")

supports_source = method_source(LOCAL_IMPORTER, "LocalFileImporter", "supports")

download_source = method_source(LOCAL_IMPORTER, "LocalFileImporter", "download")

assert (
    contains_attribute_access(local_source, "local_path") or '"local_path"' in local_source
), "Resolver does not reference canonical local_path"

assert '"source_path"' in local_source, ("Resolver does not contain metadata compatibility fallback")

assert has_call(supports_source, "_resolve_source_path"), "supports() does not delegate to _resolve_source_path()"

assert has_call(download_source, "_resolve_source_path"), "download() does not delegate to _resolve_source_path()"
...
for helper in ("destination_file", "validate_destination", "remove_existing", "finalize_result"):
...
for method_name, source in (("download", download_source),):
    for helper in ("validate_destination", "finalize_result", "destination_file", "remove_existing"):
        print(
...
result_source = ACQUISITION_RESULT.read_text(encoding="utf-8")

for method_name in ("finalize", "mark_success", "add_error", "add_downloaded_file"):
...
acquirer_source = DEFAULT_SOURCE_ACQUIRER.read_text(encoding="utf-8")
...
if "supports(" in acquirer_source:
    ...
    print("WARNING: no obvious supports() dispatch evidence")
...
if not AMARAKOSHA_ARTIFACT.is_file():
    raise RuntimeError(f"Missing canonical artifact: {AMARAKOSHA_ARTIFACT}")
...
if manifest.destination_directory is None:
    print("BLOCKER: production manifest has no destination directory")
else:
    destination_directory = Path(manifest.destination_directory)

    expected_filename = manifest.expected_filename or source_path.name

    destination_path = destination_directory / expected_filename

    print(f"source path      : {source_path}")
    print(f"destination path : {destination_path}")

    same_path = physical_same_path(source_path, destination_path)

    print("source == destination: " + ("WARNING / BLOCKER CANDIDATE" if same_path else "PASS"))
...
with TemporaryDirectory() as temp_dir:
    ...
    isolated_manifest = replace(manifest, destination_directory=destination, metadata=dict(manifest.metadata))

    importer = LocalFileImporter()

    supports = importer.supports(isolated_manifest)

    print(f"LocalFileImporter.supports(): {supports}")

    if not supports:
        raise RuntimeError("LocalFileImporter unexpectedly rejected canonical local source")

    result = importer.download(isolated_manifest)

    print(f"AcquisitionResult.success : {result.success}")

    print(f"AcquisitionResult.message : {result.message}")
    ...
    if not result.success:
        raise RuntimeError("Isolated LocalFileImporter runtime failed")

    if not result.downloaded_files:
        raise RuntimeError("Successful local acquisition produced no downloaded_files")
    ...
    print("Copied artifact size/hash: PASS")
...
print("amarakosha.txt unchanged during audit: PASS")
...
if manifest.destination_directory is not None:
    destination_path = Path(manifest.destination_directory) / (manifest.expected_filename or source_path.name)

    if physical_same_path(source_path, destination_path):
        print(
        ...
        print()
        print("Do NOT repair LocalFileImporter yet.")
    else:
        print("13R-8F-9 COMPLETE — no source/destination collision.")
else:
    print("13R-8F-9 COMPLETE — manifest destination requires review.")
