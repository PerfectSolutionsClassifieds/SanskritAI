...
def make_context(
    ...
    """
    Creates a CanonicalContext using its current canonical
    constructor contract.

    The ContextIndex uses ``context.identifier`` as the
    lookup key.  The identifier itself is therefore represented
    here through the canonical ``corpus`` field.
    """
...
def make_sense(
    ...
    """
    Creates a CanonicalDictionarySense using the current
...
def make_entry(
    ...
    CanonicalDictionaryEntry is immutable, so senses must be
    supplied through the constructor rather than through a
    mutating ``add_sense()`` method.
    """
...
def make_lexicon(
    *entries: CanonicalDictionaryEntry,
    ...
    """
    Creates a canonical lexicon using its immutable ``entries``
    mapping.
    """
...
def test_multiple_senses_are_indexed():
    ...
    assert (
        builder.context_index.lookup("context-1")
        == (sense1, sense2)
    )

    assert (
        builder.source_index.lookup("source-1")
        == (sense1, sense2)
    )
...
def test_build_processes_multiple_lexicons():
    ...
    # IMPORTANT:
    # Do not use ``assert lookup(...)`` here.
    # CanonicalDictionaryEntry implements __len__(),
    # therefore an entry with zero senses evaluates to False.
    assert (
        builder.headword_index.lookup("राम")
        is not None
    )
