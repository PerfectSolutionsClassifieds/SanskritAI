...
def test_read_based_source_is_supported() -> None:
    ...
    service = MonierWilliamsAcquisitionService(source=source)
...
def test_local_source_acquirer_reads_text(tmp_path: Path) -> None:
...
def test_local_source_acquirer_exposes_path(tmp_path: Path) -> None:
    source_file = tmp_path / "mw.txt"
...
def test_local_source_acquirer_exposes_encoding(tmp_path: Path) -> None:
    source_file = tmp_path / "mw.txt"
    source_file.write_text("source", encoding="utf-8")
...
def test_local_source_acquirer_rejects_missing_file(tmp_path: Path) -> None:
...
def test_local_source_acquirer_rejects_directory(tmp_path: Path) -> None:
    source_directory = tmp_path / "directory"
    source_directory.mkdir()

    acquirer = LocalMonierWilliamsSourceAcquirer(source_directory)
