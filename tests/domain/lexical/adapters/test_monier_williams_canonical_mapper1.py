...
def test_mapper_preserves_source_metadata():
    ...
    assert entry.metadata["source_reference"] == "MW:001"
    assert entry.metadata["grammatical_category"] == "noun"
    assert entry.metadata["homonym"] == "1"

    assert sense is not None
    assert sense.metadata["source_id"] == "MW-001"
    assert sense.metadata["source_reference"] == "MW:001"
