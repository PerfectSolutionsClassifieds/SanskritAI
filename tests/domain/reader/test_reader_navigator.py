...
repository delegation, structural-context preservation, and navigation
boundary behavior.
The tests intentionally use a repository stub so Navigator is tested
independently from Corpus construction.
"""
...
def test_sloka_navigation_uses_view_structural_context():
    ...
    repository.slokas["sloka-2"] = FakeView(
        "sloka-2",
        ReaderPositionFactory.sloka(
            purana_id="purana-view",
            chapter_id="chapter-2",
            sloka_id="sloka-2",
        ),
    )
    current = ReaderPositionFactory.sloka(
        purana_id="purana-current",
        chapter_id="chapter-1",
        sloka_id="sloka-1",
    )
    result = navigator.next_sloka(current)
