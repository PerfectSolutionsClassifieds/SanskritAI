...
try:
    ...
    from SanskritAI.services.importers.unicode_normalizer import UnicodeNormalizer
    from SanskritAI.services.importers.parser_state import ParserState
    from SanskritAI.services.importers.parser_errors import (
        RecoverableParserError,
        FatalParserError,
        StructureError,
    )
    from SanskritAI.services.importers.parser_context import ParserContext
    from SanskritAI.services.importers.amarakosha_builder import AmarakoshaBuilder
    from SanskritAI.services.importers.parser_validator import ParserValidator
    from SanskritAI.services.importers.structure_numbering import StructureNumbering
    from SanskritAI.services.importers.classification_result import ClassificationResult
    from SanskritAI.services.importers.import_result_builder import ImportResultBuilder
except ImportError:
...
class AmarakoshaParser:
    ...
    Coordinates normalization, classification, state management,
    structure construction, validation, metrics, and diagnostics.
    """
    ...
    def __init__(
        ...
        self.configuration = (
            configuration if configuration is not None else ImportConfiguration()
        )
    ...
    def _engine_loop(self, lines: list[str]) -> None:
        ...
        for raw_line in lines:
            ...
            classified = self._classifier.classify(self.context.current_line)

            if isinstance(classified, ClassificationResult):
                result = classified
            else:
                result = ClassificationResult(
                    line_type=classified,
