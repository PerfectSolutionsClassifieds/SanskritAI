...
class DefaultPratyayaStrategy(
    ...
    @property
    def repository(self) -> PratyayaRepository | None:
    ...
    def _canonical_collection(self) -> PratyayaCollection | None:
        """
        Returns the configured collection or the canonical
        factory-built collection if no explicit collection was
        supplied.
        """
        ...
        return PratyayaFactory.create_default_collection()
    ...
    def _match_pratyaya(
        ...
        for item in canonical:
            if surface.endswith(item.pratyaya):
                return item
