...
This strategy now wires VakyaParser into the normalization
path so every sentence is parsed into a stable structure and
lightweight roles before rules run.
...
from SanskritAI.domain.vakya.vakya_parser import (
    VakyaParseResult,
    VakyaParser,
)
...
class DefaultVakyaStrategy:
    ...
    def __init__(
        ...
        self._parser = parser if parser is not None else VakyaParser()
    ...
    def _parsed_structure(
        self,
        context: VakyaContext,
    ) -> VakyaParseResult:
        """
        Parses and normalizes the raw sentence input before
        rule execution.
        """
    ...
    def _normalized_context(
        ...
        """
        Normalizes the sentence input before rule execution and
        injects parser results into metadata.
        """
        parse_result = self._parsed_structure(context)
        structure = parse_result.structure

        merged_metadata = dict(context.metadata)
        merged_metadata["vakya_structure"] = structure
        merged_metadata["vakya_parse_result"] = parse_result
        merged_metadata["vakya_roles"] = parse_result.roles
        merged_metadata["vakya_role_count"] = parse_result.role_count
        merged_metadata["vakya_role_names"] = tuple(
            role.role for role in parse_result.roles
        )
        merged_metadata.setdefault(
            "normalized_sentence",
            structure.normalized_sentence,
        )
        merged_metadata.setdefault(
            "sentence_components",
