...
class ReaderSession:
    ...
    def open(
        ...
        """
        Open a new reader position.
        Opening a position establishes a new browsing root.
        Existing session history is therefore discarded.
        The supplied position becomes:
            1. the current session position
            2. the initial history entry
        This method intentionally delegates to set_position()
        so there is only one implementation of the new-session
        position contract.
        """
        result = self.set_position(
            position,
        )
        # position is explicitly non-None by contract.
